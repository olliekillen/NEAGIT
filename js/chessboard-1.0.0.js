  ...
  function constructor (containerElOrString, config) {
    ...
    function dropDraggedPieceOnSquare (square) {
      ...
      function onAnimationComplete () {
        ...
        // execute their updateBoard function
        if (isFunction(config.updateBoard)) {
          config.updateBoard(draggedPieceSource, square, draggedPiece)
        }
      }
    ...
    function beginDraggingPiece (source, piece, x, y) {
      // run their custom pickUpPiece function
      // their custom pickUpPiece function can cancel drag start
      if (isFunction(config.pickUpPiece) &&
          config.pickUpPiece(source, piece, deepCopy(currentPosition), currentOrientation) === false) {
        return
      }
    ...
    function stopDraggedPiece (location) {
      ...
      // run their dropPiece function, which can potentially change the drop action
      if (isFunction(config.dropPiece)) {
        var newPosition = deepCopy(currentPosition)
